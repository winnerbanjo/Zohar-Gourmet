...
// Topping emoji map for funky falling particles
const EMOJI_MAP = {
  apple: '🍏',
  granola: '🌾',
  coconut: '🥥',
...
export default function Customizer({ onAddToCart, onClose }) {
  ...
  // Falling particles animation state
  const [fallingParticles, setFallingParticles] = useState([]);

  // Load toppings on mount
  useEffect(() => {
    setToppingsList(database.getToppings());
  }, []);

  // Play a soft bubble pop synth sound when clicking options
  const playPopSound = () => {
    try {
  ...
  // Trigger falling emoji particle helper
  const spawnParticle = (typeKey) => {
    ...
    const newParticle = {
      ...
      emoji,
      style: {
        left: `${20 + Math.random() * 60}%`, // Random position across the cup width
        animationDelay: '0s'
      }
    };
  ...
  const handleToppingToggle = (toppingId) => {
    ...
    if (selectedToppings.includes(toppingId)) {
      ...
      spawnParticle(toppingId); // Spawn particle on add!
    }
  };
  ...
  const handleAdd = () => {
    ...
    const chosenToppings = selectedToppings.map(id => {
    ...
    const item = {
      ...
      options: `${baseLabel} with toppings: ${chosenToppings.join(', ') || 'No toppings'}`,
      isCustom: true,
      customDetails: {
        size,
        base,
  ...
  const layerCount = 1 + selectedToppings.length;
  const heightPercent = 100 / layerCount;
    ...
    <div className="customizer-split anim-scale-up">
      ...
      <div className="cup-visual-section">
        ...
        <div className="parfait-cup-container">
          ...
          {/* Falling Topping Emoji Particles */}
          {fallingParticles.map(p => (
            <div 
              key={p.id} 
              className="falling-topping" 
              style={p.style}
            >
              {p.emoji}
            </div>
            ...
            {selectedToppings.map((toppingId) => {
              ...
              return (
                <div 
                  ...
                  style={{ height: `${heightPercent}%` }}
                >
                  {topping ? topping.name.split(' ')[1] || topping.name.split(' ')[0] : toppingId}
                </div>
              );
            })}
          </div>
        ...
        <p style={{ fontSize: '12px', color: 'var(--color-gray-dark)', textAlign: 'center', marginTop: '8px' }}>
          *Click toppings to see them drop in! 🍓
        </p>
      </div>
      ...
      <div className="customizer-options">
        ...
        <div>
          <div style={{ display: 'flex', justifycontent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
            <h5 className="option-group-title" style={{ margin: 0 }}>3. Add Toppings</h5>
            <span style={{ fontSize: '11px', color: 'var(--color-gray-dark)', display: 'flex', alignItems: 'center', gap: '4px' }}>
              <Info size={12} /> Cashew & Almond: ₦700 | Others: ₦500
            </span>
          </div>
            ...
            {toppingsList.map((topping) => {
              ...
              return (
                <button
                  ...
                  className={`topping-checkbox-row ${isChecked ? 'checked' : ''}`}
                  style={{ width: '100%', cursor: isDisabled ? 'not-allowed' : 'pointer' }}
                >
                  <span className="topping-checkbox-label" style={{ opacity: isDisabled ? 0.5 : 1 }}>
                    <div style={{
                      width: '18px',
                      height: '18px',
                      borderRadius: '4px',
                  ...
                  <span className="topping-price-badge" style={{ opacity: isDisabled ? 0.5 : 1 }}>
